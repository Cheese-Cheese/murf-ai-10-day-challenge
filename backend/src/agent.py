    ...
    try:
        ...
        if cur.fetchone()[0] == 0:
            catalog = [
                ...
                ("sugar-1kg", "Madhur Sugar", "Staples", 60.00, "Madhur", "1kg", "pack", json.dumps(["sweet", "essential"])),
...
async def show_cart(ctx: RunContext[Userdata]) -> str:
    ...
    lines = []
    for ci in ctx.userdata.cart:
        lines.append(f"- {ci.quantity} unit {ci.name} @ \u20B9{ci.unit_price:.2f} each = \u20B9{ci.unit_price * ci.quantity:.2f}")
    total = cart_total(ctx.userdata.cart)
    return "Your cart:\n" + "\n".join(lines) + f"\nTotal: \u20B9{total:.2f}"
...
async def place_order(
    ...
    # 2. GENERATE JSON FILE (Requirement update)
    try:
        json_filename = f"order_{order_id}.json"
        order_data = {
            "order_id": order_id,
            "timestamp": now,
            "total": total,
            ...
            "address": address,
            "status": "received",
            # Convert CartItems to dicts using asdict
            "items": [asdict(ci) for ci in ctx.userdata.cart]
        }
        with open(json_filename, "w", encoding="utf-8") as f:
            json.dump(order_data, f, indent=4)
        logger.info(f"✅ Generated JSON order file: {json_filename}")
    except Exception as e:
        logger.error(f"⚠️ Failed to generate JSON order file: {e}")

    # 3. Clear Cart
    ctx.userdata.cart = []
    ctx.userdata.customer_name = customer_name

    # 4. Trigger Background Simulation (Received -> Shipped -> Out for delivery...)
    try:
    ...
    return f"Order placed successfully! Order ID: {order_id}. Total: \u20B9{total:.2f}. I have saved your order receipt to order_{order_id}.json and initiated express shipping."
...
class FoodAgent(Agent):
    def __init__(self):
        super().__init__(
            instructions="""
            You are 'Robin', a helpful assistant for 'The Daily Essentials Shop', an Indian grocery store.
